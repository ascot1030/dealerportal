							...
							<span class="dropdown span{if !empty($src)} settingIcons {/if}">
								{if !empty($src)}
									...
									<ul class="dropdown-menu pull-right">
										...
										{if Vtiger_Util_Helper::replaceSpaceWithUnderScores($obj->getLabel()) eq "LBL_CRM_SETTINGS" AND $USER_MODEL->get('last_name') neq "systemadmin"}
										    {assign var="subIndex" value=1}
										{else}
											{if $obj->getLabel() eq NULL AND $subIndex==0}
												<li class="divider">&nbsp;</li>
												{else if $obj->getLabel() neq NULL}
