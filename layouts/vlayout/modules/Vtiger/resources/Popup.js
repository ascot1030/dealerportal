...
jQuery.Class("Vtiger_Popup_Js",{
    ...
    registerAddBlindButton : function(){
        var popupPageContentsContainer = this.getPopupPageContainer();
        var thisInstance = this;
        popupPageContentsContainer.on('click','button.addblind', function(e){

            var blindname = "Single Roller Blind";
            var location = jQuery('#location').val();
            var quantity = jQuery.trim(jQuery('#quantity').val());
            var productwidth = jQuery.trim(jQuery('#productwidth').val());
            var productdrop = jQuery.trim(jQuery('#productdrop').val());
            var fabric = jQuery('#fabric').val();
            var fabric_description = jQuery('#fabric').find(":selected").text();
            var controltype = jQuery('#controltype').val();
            var rolldirection = jQuery('#rolldirection').val();
            var bottomrail = jQuery('#bottomrail').val();
            var railcolour = jQuery('#railcolour').val();
            var controlside = jQuery('#controlside').val();
            var controlcolour = jQuery('#controlcolour').val();
            var chaincolour = jQuery('#chaincolour').val();
            var chainlength = jQuery('#chainlength').val();
            var comments = jQuery('#comments').val();

            var springassist = 'No';
            var bracketcovers = 'No';
            var railroad = 'No';
            if ($('#springassist').is(":checked")) {
                springassist = 'Yes';
            }
            if ($('#bracketcovers').is(":checked")) {
                bracketcovers = 'Yes';
            }
            if ($('#railroad').is(":checked")) {
                railroad = 'Yes';
            }
            ...
            if(location=="") {
                ...
                alert("Please select bottom rail");
            }else if(parseInt(productwidth)>2300) {
//                alert("Width exceeds maximum air shipment sizes, split blind it two to automatically process.");


                var message = "Width exceeds maximum air shipment sizes, split blind it two to automatically process.";
                Vtiger_Helper_Js.showWidthConfirmationBox({'message' : message}).then(
                    function(e) {
                        thisInstance.addBlinds(blindname,location,quantity,productwidth,productdrop,fabric,
                            controltype,rolldirection,bottomrail,railcolour,
                            controlside,controlcolour,chaincolour,chainlength,
                            springassist,bracketcovers,railroad,comments,fabric_description);
                    },
                    function(error, err){
                    });
            }else {
                ...
                thisInstance.addBlinds(blindname,location,quantity,productwidth,productdrop,fabric,
                                     ...
                                     springassist,bracketcovers,railroad,comments,fabric_description);
//
//                var dataUrl = 'index.php?module=Inventory&action=AddBlind&record=8'+'&currency_id='+jQuery('#currencyId').val();
//                dataUrl = dataUrl+'&name='+blindname+'&location='+location+'&quantity='+quantity+'&productwidth='+productwidth+'&productdrop='+productdrop+'&fabric='+fabric;
//                dataUrl = dataUrl+'&controltype='+controltype+'&rolldirection='+rolldirection+'&bottomrail='+bottomrail+'&railcolour='+railcolour;
//                dataUrl = dataUrl+'&controlside='+controlside+'&controlcolour='+controlcolour+'&chaincolour='+chaincolour+'&chainlength='+chainlength;
//                dataUrl = dataUrl+'&springassist='+springassist+'&bracketcovers='+bracketcovers+'&railroad='+railroad+'&comments='+comments;
//                dataUrl = dataUrl+'&fabric_description='+fabric_description;
//
//                AppConnector.request(dataUrl).then(
//                    function (data) {
//                        for (var id in data) {
...
//                );
    ...
    addBlinds : function(blindname,location,quantity,productwidth,productdrop,fabric,
        controltype,rolldirection,bottomrail,railcolour,
        controlside,controlcolour,chaincolour,chainlength,
        springassist,bracketcovers,railroad,comments,fabric_description) {
        var thisInstance = this;
