...
class Inventory_Record_Model extends Vtiger_Record_Model {
	...
	function getRetailProductTaxes() {
		$taxDetails = $this->get('retail_taxDetails');
		if ($taxDetails) {
			return $taxDetails;
		}
		...
		if ($record) {
			$relatedProducts = getAssociatedProducts($this->getModuleName(), $this->getEntity());
			$taxDetails = $relatedProducts[1]['final_details']['retail_taxes'];
		} else {
			$taxDetails = getAllTaxes('available', '', $this->getEntity()->mode, $this->getId());
		}

		$this->set('retail_taxDetails', $taxDetails);
		return $taxDetails;
	}
	...
	function getRetailShippingTaxes() {
		$shippingTaxDetails = $this->get('retail_shippingTaxDetails');
		if ($shippingTaxDetails) {
			return $shippingTaxDetails;
		}
		...
		if ($record) {
			$relatedProducts = getAssociatedProducts($this->getModuleName(), $this->getEntity());
			$shippingTaxDetails = $relatedProducts[1]['final_details']['retail_sh_taxes'];
		} else {
			$shippingTaxDetails = getAllTaxes('available', 'sh', 'edit', $this->getId());
		}

		$this->set('retail_shippingTaxDetails', $shippingTaxDetails);
		return $shippingTaxDetails;
	}
...
/*
        //jin
        for($i=1;$i<=$productsCount;$i++) {
            $netPrice = $relatedProducts[$i]['netPrice'.$i];
            $qty = $relatedProducts[$i]['qty'.$i];
            $totalEx = $relatedProducts[1]['final_details']['hdnSubTotal'];
//            $totalEx = $relatedProducts[1]['final_details']['totalAfterDiscount'];
            $preTaxTotal = $relatedProducts[1]['final_details']['preTaxTotal'];
            $whlPrice = $netPrice/$totalEx*$preTaxTotal/$qty;
            $relatedProducts[$i]['whlPrice'.$i] = number_format($whlPrice, getCurrencyDecimalPlaces(),'.','');
        }
*/
//		echo "Jin<pre>";
