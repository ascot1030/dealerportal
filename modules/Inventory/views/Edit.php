...
Class Inventory_Edit_View extends Vtiger_Edit_View {
	...
	public function process(Vtiger_Request $request) {
		...
		if(!empty($record)  && $request->get('isDuplicate') == true) {
			...
			$retail_taxes = $recordModel->getRetailProductTaxes();
			$shippingTaxes = $recordModel->getShippingTaxes();
			$retail_shippingTaxes = $recordModel->getRetailShippingTaxes();
			...
			$retail_taxes = $parentRecordModel->getRetailProductTaxes();
			$shippingTaxes = $parentRecordModel->getShippingTaxes();
			$retail_shippingTaxes = $parentRecordModel->getRetailShippingTaxes();
			$relatedProducts = $parentRecordModel->getProducts();
			$recordModel = Vtiger_Record_Model::getCleanInstance($moduleName);
			$recordModel->setRecordFieldValues($parentRecordModel);
		} else {
			$taxes = Inventory_Module_Model::getAllProductTaxes();
			$retail_taxes = Inventory_Module_Model::getAllProductTaxes(); //jin need to confirm
			$shippingTaxes = Inventory_Module_Model::getAllShippingTaxes();
			$retail_shippingTaxes = Inventory_Module_Model::getAllShippingTaxes(); //jin
			$recordModel = Vtiger_Record_Model::getCleanInstance($moduleName);
			$viewer->assign('MODE', '');
		...
		$viewer->assign('RETAIL_SHIPPING_TAXES', $retail_shippingTaxes);
		$viewer->assign('TAXES', $taxes);
		$viewer->assign('RETAIL_TAXES', $retail_taxes);
        ...
        if($depth<3) {
            $viewer->view('EditViewAll.tpl', 'Inventory');
        } else if($depth==3) {
		    $viewer->view('EditView.tpl', 'Inventory');
		} else {
		    $viewer->view('EditViewRetail.tpl', 'Inventory');
		}
	}
