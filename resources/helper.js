...
jQuery.Class("Vtiger_Helper_Js",{
    ...
    showWidthConfirmationBox : function(data){
        var aDeferred = jQuery.Deferred();
        var bootBoxModal = bootbox.confirm(data['message'],app.vtranslate('LBL_CANCEL'),app.vtranslate('LBL_PROCEED'), function(result) {
            if(result){
                aDeferred.resolve();
            } else{
                aDeferred.reject();
