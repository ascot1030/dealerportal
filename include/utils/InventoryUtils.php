...
//echo "</pre>";
	...
	for($i=1; $i<=$tot_no_prod; $i++)
	{
        ...
        $productwidth = vtlib_purify($_REQUEST['productwidth'.$i]);
        $productdrop = vtlib_purify($_REQUEST['productdrop'.$i]);
        $location = vtlib_purify($_REQUEST['location'.$i]);
        $fabric = vtlib_purify($_REQUEST['fabric'.$i]);

        $controltype = vtlib_purify($_REQUEST['controltype'.$i]);
        $rolldirection = vtlib_purify($_REQUEST['rolldirection'.$i]);
        $bottomrail = vtlib_purify($_REQUEST['bottomrail'.$i]);
        $railcolour = vtlib_purify($_REQUEST['railcolour'.$i]);
        $controlside = vtlib_purify($_REQUEST['controlside'.$i]);
        $controlcolour = vtlib_purify($_REQUEST['controlcolour'.$i]);
        $chaincolour = vtlib_purify($_REQUEST['chaincolour'.$i]);
        $chainlength = vtlib_purify($_REQUEST['chainlength'.$i]);
        $springassist = vtlib_purify($_REQUEST['springassist'.$i]);
        $bracketcovers = vtlib_purify($_REQUEST['bracketcovers'.$i]);
        $railroad = vtlib_purify($_REQUEST['railroad'.$i]);
        $comments = vtlib_purify($_REQUEST['comments'.$i]);

        $purchaseCost = vtlib_purify($_REQUEST['purchaseCost'.$i]); 
        $margin = vtlib_purify($_REQUEST['margin'.$i]);

        $wholesalePrice = vtlib_purify($_REQUEST['wholesalePrice'.$i]);
        $retail_listPrice = vtlib_purify($_REQUEST['retail_listPrice'.$i]);
		...
		$query ="insert into vtiger_inventoryproductrel(id, productid, sequence_no, quantity, listprice, comment, description, productwidth, productdrop, location, fabric, controltype, rolldirection, bottomrail, railcolour, controlside, controlcolour, chaincolour, chainlength, springassist, bracketcovers, railroad, comments,";
		$query.="wholesaleprice,retail_listprice) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		$qparams = array($focus->id,$prod_id,$prod_seq,$qty,$listprice,$comment,$description,$productwidth,$productdrop,$location,$fabric,$controltype,$rolldirection,$bottomrail,$railcolour,$controlside,$controlcolour,$chaincolour,$chainlength,$springassist,$bracketcovers,$railroad,$comments,$wholesalePrice,$retail_listPrice);
		$adb->pquery($query,$qparams);
        ...
        if($_REQUEST['retail_discount_type'.$i] == 'percentage')
        {
            $updatequery .= " retail_discount_percent=?,";
            array_push($updateparams, $_REQUEST['retail_discount_percentage'.$i]);
        }
        elseif($_REQUEST['retail_discount_type'.$i] == 'amount')
        {
            $updatequery .= " retail_discount_amount=?,";
            $discount_amount = $_REQUEST['retail_discount_amount'.$i];
            array_push($updateparams, $discount_amount);
        }
	...
	$retail_subtotal = $_REQUEST['retail_subtotal'];
	$updatequery .= " retail_subtotal=?,";
    array_push($updateparams, $retail_subtotal);
	...
	if($_REQUEST['retail_discount_type_final'] == 'percentage')
	{
		$updatequery .= " retail_discount_percent=?,retail_discount_amount=?,";
        array_push($updateparams, vtlib_purify($_REQUEST['retail_discount_percentage_final']));
        array_push($updateparams,null);
	}
	elseif($_REQUEST['retail_discount_type_final'] == 'amount')
	{
        $discount_amount_final = vtlib_purify($_REQUEST['retail_discount_amount_final']);
        $updatequery .= " retail_discount_amount=?,retail_discount_percent=?,";
        array_push($updateparams, $discount_amount_final);
        array_push($updateparams,null);
    }
    elseif($_REQUEST['retail_discount_type_final']=='zero'){
        $updatequery.="retail_discount_amount=?,retail_discount_percent=?,";
        array_push($updateparams,null);
        array_push($updateparams,null);
    }
    ...
    $shipping_handling_charge = vtlib_purify($_REQUEST['retail_shipping_handling_charge']);
    $updatequery .= " retail_s_h_amount=?,";
	array_push($updateparams, $shipping_handling_charge);
    ...
    $adjustmentType = '';
    if($_REQUEST['retail_adjustmentType'] == '-')
        $adjustmentType = vtlib_purify($_REQUEST['retail_adjustmentType']);

    $adjustment = vtlib_purify($_REQUEST['retail_adjustment']);
    $updatequery .= " retail_adjustment=?,";
    array_push($updateparams, $adjustmentType.$adjustment);
	...
	$total = vtlib_purify($_REQUEST['retail_total']);
	$updatequery .= " retail_total=?,";
	array_push($updateparams, $total);

	//retail_pre_tax_total
	$retail_pre_tax_total = vtlib_purify($_REQUEST['retail_pre_tax_total']);
	$updatequery .= " retail_pre_tax_total=?,";
	array_push($updateparams, $retail_pre_tax_total);

    //Retail Margin
    $retailMargin = vtlib_purify($_REQUEST['retail_margin']);
    $updatequery .= " retail_margin=?,";
    array_push($updateparams, $retailMargin);

	//Actual Discount
	$actualDiscount = vtlib_purify($_REQUEST['retail_actual_discount']);
	$updatequery .= " retail_actual_discount=?,";
	array_push($updateparams, $actualDiscount);

    //Actual Margin
    $actualMargin = vtlib_purify($_REQUEST['retail_actual_margin']);
    $updatequery .= " retail_actual_margin=?,";
    array_push($updateparams, $actualMargin);
