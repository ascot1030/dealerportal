...
function getAssociatedProducts($module,$focus,$seid='')
{
	...
	for($i=1;$i<=$num_rows;$i++)
	{
        ...
        $wholesaleprice=$adb->query_result($result,$i-1,'wholesaleprice');
		$retail_listprice=$adb->query_result($result,$i-1,'retail_listprice');
		//************Jin******************
		$productwidth=$adb->query_result($result,$i-1,'productwidth');
		$productdrop=$adb->query_result($result,$i-1,'productdrop');
		$fabric=$adb->query_result($result,$i-1,'fabric');
		$location=$adb->query_result($result,$i-1,'location');

        $controltype = $adb->query_result($result,$i-1,'controltype');
        $rolldirection = $adb->query_result($result,$i-1,'rolldirection');
        $bottomrail = $adb->query_result($result,$i-1,'bottomrail');
        $railcolour = $adb->query_result($result,$i-1,'railcolour');
        $controlside = $adb->query_result($result,$i-1,'controlside');
        $controlcolour = $adb->query_result($result,$i-1,'controlcolour');
        $chaincolour = $adb->query_result($result,$i-1,'chaincolour');
        $chainlength = $adb->query_result($result,$i-1,'chainlength');
        $springassist = $adb->query_result($result,$i-1,'springassist');
        $bracketcovers = $adb->query_result($result,$i-1,'bracketcovers');
        $railroad = $adb->query_result($result,$i-1,'railroad');
        $comments = $adb->query_result($result,$i-1,'comments');

/*
        if($springassist=='true') {
            $springassist = 'Yes';
        } else {
            $springassist = 'No';
        }
        if($bracketcovers=='true') {
            $bracketcovers = 'Yes';
        } else {
            $bracketcovers = 'No';
        }
...
*/
        ...
        if($retail_listprice == '')
            $retail_listprice = $wholesaleprice;
		...
		//******************Jin*****************
		if($module == 'Quotes' || $module == 'PurchaseOrder' || $module == 'SalesOrder' || $module == 'Invoice') {
		    $product_Detail[$i]['productwidth'.$i]= $productwidth;
		    $product_Detail[$i]['productdrop'.$i]= $productdrop;
		    $product_Detail[$i]['fabric'.$i]= $fabric;
		    $product_Detail[$i]['location'.$i]= $location;

		    $product_Detail[$i]['controltype'.$i]= $controltype;
		    $product_Detail[$i]['rolldirection'.$i]= $rolldirection;
		    $product_Detail[$i]['bottomrail'.$i]= $bottomrail;
		    $product_Detail[$i]['railcolour'.$i]= $railcolour;
		    $product_Detail[$i]['controlside'.$i]= $controlside;
		    $product_Detail[$i]['controlcolour'.$i]= $controlcolour;
		    $product_Detail[$i]['chaincolour'.$i]= $chaincolour;
		    $product_Detail[$i]['chainlength'.$i]= $chainlength;
		    $product_Detail[$i]['springassist'.$i]= $springassist;
		    $product_Detail[$i]['bracketcovers'.$i]= $bracketcovers;
		    $product_Detail[$i]['railroad'.$i]= $railroad;
		    $product_Detail[$i]['comments'.$i]= $comments;

            $query_fabric = "SELECT * FROM product_field_fabric WHERE fabricid=?";
            $params_fabric = array($fabric);
            $result_fabric = $adb->pquery($query_fabric, $params_fabric);
            $row_fabric = $adb->query_result_rowdata($result_fabric, 0);
            $product_Detail[$i]['fabric_description'.$i] = $row_fabric['fabric'];
		}
		...
		$wholesaleprice = number_format($wholesaleprice, $no_of_decimal_places,'.','');
		$retail_listprice = number_format($retail_listprice, $no_of_decimal_places,'.','');
		$unitprice = number_format($unitprice, $no_of_decimal_places,'.','');
		$productTotal = number_format($productTotal, $no_of_decimal_places,'.','');
		$retail_productTotal = number_format($retail_productTotal, $no_of_decimal_places,'.','');

		$product_Detail[$i]['qty'.$i]=decimalFormat($qty);
		$product_Detail[$i]['listPrice'.$i]=$listprice;
		$product_Detail[$i]['wholesalePrice'.$i]=$wholesaleprice;
		$product_Detail[$i]['retail_listPrice'.$i]=$retail_listprice;
		$product_Detail[$i]['unitPrice'.$i]=$unitprice;
		$product_Detail[$i]['productTotal'.$i]=$productTotal;
		$product_Detail[$i]['retail_productTotal'.$i]=$retail_productTotal;
        ...
        $retail_discount_percent = decimalFormat($adb->query_result($result,$i-1,'retail_discount_percent'));
        $retail_discount_amount = $adb->query_result($result,$i-1,'retail_discount_amount');
        $retail_discount_amount = decimalFormat(number_format($retail_discount_amount, $no_of_decimal_places,'.',''));
		...
		$product_Detail[$i]['retail_discount_percent'.$i] = 0;
		$product_Detail[$i]['retail_discount_amount'.$i] = 0;
		if(!empty($retail_discount_percent)) {
			$product_Detail[$i]['retail_discount_type'.$i] = "percentage";
			$product_Detail[$i]['retail_discount_percent'.$i] = $retail_discount_percent;
			$product_Detail[$i]['retail_checked_discount_percent'.$i] = ' checked';
			$product_Detail[$i]['retail_style_discount_percent'.$i] = ' style="visibility:visible"';
			$product_Detail[$i]['retail_style_discount_amount'.$i] = ' style="visibility:hidden"';
			$retail_discountTotal = $retail_productTotal*$retail_discount_percent/100;
		} elseif(!empty($retail_discount_amount)) {
			$product_Detail[$i]['retail_discount_type'.$i] = "amount";
			$product_Detail[$i]['retail_discount_amount'.$i] = $retail_discount_amount;
			$product_Detail[$i]['retail_checked_discount_amount'.$i] = ' checked';
			$product_Detail[$i]['retail_style_discount_amount'.$i] = ' style="visibility:visible"';
			$product_Detail[$i]['retail_style_discount_percent'.$i] = ' style="visibility:hidden"';
			$retail_discountTotal = $retail_discount_amount;
		}
		else
		{
			$product_Detail[$i]['retail_checked_discount_zero'.$i] = ' checked';
		}
		$retail_totalAfterDiscount = $retail_productTotal-$retail_discountTotal;
		$retail_totalAfterDiscount = number_format($retail_totalAfterDiscount, $no_of_decimal_places,'.','');
		$retail_discountTotal = number_format($retail_discountTotal, $no_of_decimal_places,'.','');
		$product_Detail[$i]['retail_discountTotal'.$i] = $retail_discountTotal;
		$product_Detail[$i]['retail_totalAfterDiscount'.$i] = $retail_totalAfterDiscount;

		$retail_taxTotal = '0.00';
		$retail_taxTotal = number_format($retail_taxTotal, $no_of_decimal_places,'.','');
		$product_Detail[$i]['retail_taxTotal'.$i] = $retail_taxTotal;
		...
		$netPrice = number_format($netPrice, $no_of_decimal_places,'.','');
		$product_Detail[$i]['netPrice'.$i] = $netPrice;

		$retail_netPrice = $retail_totalAfterDiscount+$retail_taxTotal;
		//if condition is added to call this function when we create PO/SO/Quotes/Invoice from Product module
		if($module == 'PurchaseOrder' || $module == 'SalesOrder' || $module == 'Quotes' || $module == 'Invoice')
		{
			$retail_taxtype = getInventoryTaxType($module,$focus->id);
			if($retail_taxtype == 'individual')
			{
				//Add the tax with product total and assign to netprice
				$retail_netPrice = $retail_netPrice+$retail_taxTotal;
			}
		}
		$retail_netPrice = number_format($retail_netPrice, $no_of_decimal_places,'.','');
		$product_Detail[$i]['retail_netPrice'.$i] = $retail_netPrice;
    ...
    $product_Detail[1]['final_details']['totalqty'] = $totalqty;
	//set the taxtype
	...
	elseif(!empty($focus->column_fields['hdnDiscountAmount']))
	{
		...
		$product_Detail[1]['final_details']['discount_amount_final'] = number_format($discountAmount, $no_of_decimal_places,'.','');
	...
	$product_Detail[1]['final_details']['tax_totalamount'] = $product_Detail[1]['final_details']['tax_totalamount']+$product_Detail[1]['final_details']['shtax_totalamount'];
...
//	for($i=1;$i<=$num_rows;$i++) {
//        $product_Detail[$i]['whlPrice'.$i] = $product_Detail[$i]['netPrice'.$i];
//        $product_Detail[$i]['whlPrice'.$i] = $product_Detail[$i]['qty'.$i];
//        $product_Detail[$i]['whlPrice'.$i] = $product_Detail[1]['final_details']['hdnSubTotal'];
//        $product_Detail[$i]['whlPrice'.$i] = $product_Detail[1]['final_details']['preTaxTotal'];
//	}

//*****************RETAIL*****************************

    //Retail PreTaxTotal
    $retail_pre_tax_total = ($focus->column_fields['retail_pre_tax_total'] != '')?$focus->column_fields['retail_pre_tax_total']:'00.00';
    $retail_pre_tax_total = number_format($retail_pre_tax_total, $no_of_decimal_places,'.','');
    $product_Detail[1]['final_details']['retail_pre_tax_total'] = $retail_pre_tax_total;

    //Retail Margin
    $retail_margin = ($focus->column_fields['retail_margin'] != '')?$focus->column_fields['retail_margin']:'50.00';
    $retail_margin = number_format($retail_margin, $no_of_decimal_places,'.','');
    $product_Detail[1]['final_details']['retail_margin'] = $retail_margin;

    //Retail Actual Margin
    $retail_actual_margin = ($focus->column_fields['retail_actual_margin'] != '')?$focus->column_fields['retail_actual_margin']:'00.00';
    $retail_actual_margin = number_format($retail_actual_margin, $no_of_decimal_places,'.','');
    $product_Detail[1]['final_details']['retail_actual_margin'] = $retail_actual_margin;

    //Retail Actual Discount
    $retail_actual_discount = ($focus->column_fields['retail_actual_discount'] != '')?$focus->column_fields['retail_actual_discount']:'00.00';
    $retail_actual_discount = number_format($retail_actual_discount, $no_of_decimal_places,'.','');
    $product_Detail[1]['final_details']['retail_actual_discount'] = $retail_actual_discount;


	//set the taxtype
//	$product_Detail[1]['final_details']['retail_taxtype'] = $taxtype;

	//Get the Final Discount, S&H charge, Tax for S&H and Adjustment values
	//To set the Final Discount details
	$finalDiscount = '0.00';
	$product_Detail[1]['final_details']['retail_discount_type_final'] = 'zero';

	$subTotal = ($focus->column_fields['retail_hdnSubTotal'] != '')?$focus->column_fields['retail_hdnSubTotal']:'0.00';
	$subTotal = number_format($subTotal, $no_of_decimal_places,'.','');

	$product_Detail[1]['final_details']['retail_hdnSubTotal'] = $subTotal;
	$discountPercent = ($focus->column_fields['retail_hdnDiscountPercent'] != '')?$focus->column_fields['retail_hdnDiscountPercent']:'0.00';
	$discountAmount = ($focus->column_fields['retail_hdnDiscountAmount'] != '')?$focus->column_fields['retail_hdnDiscountAmount']:'0.00';
    if($discountPercent != '0'){
        $discountAmount = ($product_Detail[1]['final_details']['retail_hdnSubTotal'] * $discountPercent / 100);
    }

	//To avoid NaN javascript error, here we assign 0 initially to' %of price' and 'Direct Price reduction'(For Final Discount)
	$discount_amount_final = '0.00';
	$discount_amount_final = number_format($discount_amount_final, $no_of_decimal_places,'.','');
    $product_Detail[1]['final_details']['retail_discount_percentage_final'] = 0;
	$product_Detail[1]['final_details']['retail_discount_amount_final'] = $discount_amount_final;

	//fix for opensource issue not saving invoice data properly
	if(!empty($focus->column_fields['retail_hdnDiscountPercent']))
	{
		$finalDiscount = ($subTotal*$discountPercent/100);
		$product_Detail[1]['final_details']['retail_discount_type_final'] = 'percentage';
		$product_Detail[1]['final_details']['retail_discount_percentage_final'] = $discountPercent;
		$product_Detail[1]['final_details']['retail_checked_discount_percentage_final'] = ' checked';
		$product_Detail[1]['final_details']['retail_style_discount_percentage_final'] = ' style="visibility:visible"';
		$product_Detail[1]['final_details']['retail_style_discount_amount_final'] = ' style="visibility:hidden"';
	}
	//fix for opensource issue not saving invoice data properly
	elseif(!empty($focus->column_fields['retail_hdnDiscountAmount']))
	{
		$finalDiscount = $focus->column_fields['retail_hdnDiscountAmount'];
		$product_Detail[1]['final_details']['retail_discount_type_final'] = 'amount';
		$product_Detail[1]['final_details']['retail_discount_amount_final'] = number_format($discountAmount, $no_of_decimal_places,'.','');
		$product_Detail[1]['final_details']['retail_checked_discount_amount_final'] = ' checked';
		$product_Detail[1]['final_details']['retail_style_discount_amount_final'] = ' style="visibility:visible"';
		$product_Detail[1]['final_details']['retail_style_discount_percentage_final'] = ' style="visibility:hidden"';
	}
	$finalDiscount = number_format($finalDiscount, $no_of_decimal_places,'.','');
	$product_Detail[1]['final_details']['retail_discountTotal_final'] = $finalDiscount;
	...
	for($tax_count=0;$tax_count<count($tax_details);$tax_count++)
	{
		...
		$taxamount = number_format($taxamount, $no_of_decimal_places,'.','');
		$taxtotal = $taxtotal + $taxamount;
		$product_Detail[1]['final_details']['retail_taxes'][$tax_count]['taxname'] = $tax_name;
		$product_Detail[1]['final_details']['retail_taxes'][$tax_count]['taxlabel'] = $tax_label;
		$product_Detail[1]['final_details']['retail_taxes'][$tax_count]['percentage'] = $tax_percent;
		$product_Detail[1]['final_details']['retail_taxes'][$tax_count]['amount'] = $taxamount;
	}
	$product_Detail[1]['final_details']['retail_tax_totalamount'] = $taxtotal;

	//To set the Shipping & Handling charge
	$shCharge = ($focus->column_fields['retail_hdnS_H_Amount'] != '')?$focus->column_fields['retail_hdnS_H_Amount']:'0.00';
	$shCharge = number_format($shCharge, $no_of_decimal_places,'.','');
	$product_Detail[1]['final_details']['retail_shipping_handling_charge'] = $shCharge;
	...
	for($shtax_count=0;$shtax_count<count($shtax_details);$shtax_count++)
	{
		...
		$shtaxamount = $shCharge*$shtax_percent/100;
		$shtaxtotal = $shtaxtotal + $shtaxamount;
		$product_Detail[1]['final_details']['retail_sh_taxes'][$shtax_count]['taxname'] = $shtax_name;
		$product_Detail[1]['final_details']['retail_sh_taxes'][$shtax_count]['taxlabel'] = $shtax_label;
		$product_Detail[1]['final_details']['retail_sh_taxes'][$shtax_count]['percentage'] = $shtax_percent;
		$product_Detail[1]['final_details']['retail_sh_taxes'][$shtax_count]['amount'] = $shtaxamount;
	}
	$shtaxtotal = number_format($shtaxtotal, $no_of_decimal_places,'.','');
	$product_Detail[1]['final_details']['retail_shtax_totalamount'] = $shtaxtotal;

	$product_Detail[1]['final_details']['retail_tax_totalamount'] = $product_Detail[1]['final_details']['retail_tax_totalamount']+$product_Detail[1]['final_details']['retail_shtax_totalamount'];

	//To set the Adjustment value
	$adjustment = ($focus->column_fields['retail_txtAdjustment'] != '')?$focus->column_fields['retail_txtAdjustment']:'0.00';
	$adjustment = number_format($adjustment, $no_of_decimal_places,'.','');
	$product_Detail[1]['final_details']['retail_adjustment'] = $adjustment;

	//To set the grand total
	$grandTotal = ($focus->column_fields['retail_hdnGrandTotal'] != '')?$focus->column_fields['retail_hdnGrandTotal']:'0.00';
	$grandTotal = number_format($grandTotal, $no_of_decimal_places,'.','');
	$product_Detail[1]['final_details']['retail_grandTotal'] = $grandTotal;
